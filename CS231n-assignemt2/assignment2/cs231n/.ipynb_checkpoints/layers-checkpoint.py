...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        # *****START OF YOUR CODE (DO NO,,/T DELETE/MODIFY THIS LINE)*****
        
        sample_mean=np.mean(x,axis=0)
        sample_var=np.var(x,axis=0)+eps
        sample_st_dev=np.sqrt(sample_var)
        
        x_norm=(x-sample_mean)/(sample_st_dev)
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    sample_mean=np.expand_dims(np.transpose(np.mean(x,axis=1)),axis=1)
    sample_var=np.expand_dims(np.transpose((np.var(x,axis=1)+eps)),axis=1)
    sample_st_dev=np.sqrt(sample_var)
...
def layernorm_backward(dout, cache):
    ...
    cache=list(cache)
    cache[4] = np.expand_dims(gamma,axis=-1)
    cache[1] =  np.transpose(x_norm)
    cache[3] = np.transpose(sample_var)
    cache=tuple(cache)
    
    # transpose dout because we transposed original input, x, in forward call
    dx, dgamma, dbeta = batchnorm_backward_alt(np.transpose(dout), cache)
    # transpose gradients w.r.t. input, x, to their original dims
    dx=np.transpose(dx)
    
    dgamma= np.sum(np.transpose(dout*x_norm),axis=1)
    dbeta=  np.sum(np.transpose(dout),axis=1)
